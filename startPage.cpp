  ...
  QWizardPage (parent)
{
  ...
  QLabel *label = new QLabel (tr ("czmilPfmFilter is a tool for filtering a PFM that has been built from CZMIL data. "
                                  "It uses the dreaded Hockey Puck of Confidence (HPC).  Unlike the HPC in the editor this "
                                  "version only does one iteration through the data.  This wizard "
                                  "allows the user to set the height and radius of the hockey puck as well as the "
                                  "required number of neighbors and the required number of multi-line neighbors.  There are "
                                  "three sets of these.  The first set is for low density bathy data and has the minimum "
                                  "density for filtering.  The second set is for high density bathy data, and the third set is "
                                  "for topo data.  Bathy data are defined by classifications of 40 or 42.  Water surface data "
                                  "(41) is ignored. All other data is considered topo data.<br><br>"
                                  "Data that does not meet the HPC criteria will be flagged as PFM_SUSPECT or marked as "
...
void startPage::slotPFMFileBrowse ()
{
  ...
  QFileDialog *fd = new QFileDialog (this, tr ("czmilPfmFilter Open PFM Structure"));
  ...
  if (fd->exec () == QDialog::Accepted)
    {
      ...
      if (open_args.head.proj_data.projection)
        {
          QMessageBox::warning (this, tr ("Open PFM Structure"),
                                tr ("Sorry, czmilPfmFilter only handles geographic PFM structures."));
          close_pfm_file (pfm_handle);
        }
...
void startPage::slotAreaFileBrowse ()
{
  ...
  QFileDialog *fd = new QFileDialog (this, tr ("czmilPfmFilter Area File"));
  fd->setViewMode (QFileDialog::List);
          ...
          if (area_file_name.endsWith (".shp", Qt::CaseInsensitive))
            {
              ...
              if (shpHandle == NULL)
                {
                  QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Cannot open shape file %1!").arg (area_file_name));
                  return;
                }
              else
                {
                  ...
                  if (type != SHPT_POLYGON && type != SHPT_POLYGONZ && type != SHPT_POLYGONM &&
                    ...
                    {
                      QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Shape file %1 is not a polygon or polyline file!").arg (area_file_name));
                      return;
                    }
                  else
                    {
                      ...
                      if (shape->nVertices < 3)
                        {
                          SHPClose (shpHandle);
                          QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Number of vertices (%1) of shape file %2 is too few for a polygon!").arg
                          ...
                          if (shape->padfX[j] < -360.0 || shape->padfX[j] > 360.0 || shape->padfY[j] < -90.0 || shape->padfY[j] > 90.0)
                            {
                              ...
                              SHPClose (shpHandle);
                              QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Shape file %1 does not appear to be geographic!").arg (area_file_name));
                              return;
                            }
                        }
...
void startPage::slotExFileBrowse ()
{
  ...
  QFileDialog *fd = new QFileDialog (this, tr ("czmilPfmFilter Exclusion Area File"));
  fd->setViewMode (QFileDialog::List);
          ...
          if (ex_file_name.endsWith (".shp", Qt::CaseInsensitive))
            {
              ...
              if (shpHandle == NULL)
                {
                  QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Cannot open shape file %1!").arg (ex_file_name));
                  return;
                }
              else
                {
                  ...
                  if (type != SHPT_POLYGON && type != SHPT_POLYGONZ && type != SHPT_POLYGONM &&
                      type != SHPT_ARC && type != SHPT_ARCZ && type != SHPT_ARCM)
                    {
                      QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Shape file %1 is not a polygon or polyline file!").arg (ex_file_name));
                      return;
                    }
                  else
                    {
                          ...
                          if (shape->padfX[j] < -360.0 || shape->padfX[j] > 360.0 || shape->padfY[j] < -90.0 || shape->padfY[j] > 90.0)
                            {
                              SHPDestroyObject (shape);
                              SHPClose (shpHandle);
                              QMessageBox::warning (this, tr ("czmilPfmFilter"), tr ("Shape file %1 does not appear to be geographic!").arg (ex_file_name));
                              return;
                            }
                        }
